<?php
	...
	class ipbwi_topic extends ipbwi {
		...
		public function create($forumID,$title,$post,$desc=false,$useEmo=true,$useSig=true,$bypassPerms=false,$guestName=false,$authorID=false, $htmlState = false, $time = false){
			...
			if($row = $this->ipbwi->ips_wrapper->DB->fetch($forum)){
				...
				if($this->ipbwi->forum->isStartable($row['id']) OR $bypassPerms){
					...
					$postName	= $this->ipbwi->ips_wrapper->DB->addSlashes($this->ipbwi->makeSafe($postName));
					$postQuery = $this->ipbwi->ips_wrapper->DB->query('INSERT INTO '.$this->ipbwi->board['sql_tbl_prefix'].'posts (author_id, author_name, use_emo, use_sig, ip_address, post_date, post, queued, topic_id, post_key, new_topic, post_htmlstate) VALUES ("'.$postAuthorID.'", "'.$postName.'", "'.($useEmo ? 1 : 0).'", "'.($useSig ? 1 : 0).'", "'.$_SERVER['REMOTE_ADDR'].'", "'.$time.'", "'.$post.'", "0", "'.$topicID.'", "'.md5(microtime()).'", "1", "'.$htmlState.'")');
		 ...
		 * @param	string	$title Topic-Title
		 * @param	string	$post Topic-Post
		 * @param	string	$desc deprecated, don't use
		...
		public function edit($topicID, $title, $post, $desc=false, $reason=false, $close=false, $pin=false, $approve=true, $useEmo=true, $useSig=true, $bypassPerms=false){
			...
			if($row = $this->ipbwi->ips_wrapper->DB->fetch()){
				if($user = $this->ipbwi->member->info()){
					...
					$post	= $this->ipbwi->ips_wrapper->editor->process($apost);
					$post	= $this->ipbwi->ips_wrapper->DB->addSlashes($this->ipbwi->makeSafe($this->ipbwi->bbcode->bbcode2html($post)));
					$this->ipbwi->ips_wrapper->DB->query('UPDATE '.$this->ipbwi->board['sql_tbl_prefix'].'topics SET title="'.$ptitle.'", state="'.$state.'", pinned="'.($pin ? 1 : 0).'", topic_open_time="'.$opened.'", topic_close_time="'.$closed.'", approved="'.($approve ? 1 : 0).'" WHERE tid="'.$topicID.'"');
					...
					// update cache
					$this->ipbwi->ips_wrapper->DB->query('REPLACE INTO '.$this->ipbwi->board['sql_tbl_prefix'].'content_cache_posts (cache_content_id, cache_content, cache_updated) VALUES ("'.$row['topic_firstpost'].'", "'.$post.'", "'.time().'")');
		...
		public function info($topicID, $countView = false,$replacePostVars = true, $ipbwiLink = false, $list = false){
			...
			while($row = $this->ipbwi->ips_wrapper->DB->fetch($sql)){
				...
				$topic[$row['topic_id']]['post']				= $this->ipbwi->ips_wrapper->parser->display($topic[$row['topic_id']]['post']);
				...
				if($replacePostVars === true){
					...
					if(is_array($attachInfo) && count($attachInfo) > 0){
						foreach($attachInfo as $attachList){
							if(isset($attachList['attach_id']) && strpos($topic[$row['topic_id']]['post'],'[attachment='.$attachList['attach_id'].':') != false) {
