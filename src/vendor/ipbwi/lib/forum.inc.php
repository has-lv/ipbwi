					...
					if($subqery = $this->ipbwi->ips_wrapper->DB->query($sql)){
						...
						while($row = $this->ipbwi->ips_wrapper->DB->fetch($subqery)){
							if($outputType == 'array_ids_only'){
								...
								$subforums[$row['id']]['name'] = $row['name'];
							}else{
								if(isset($row['last_poster_name'])){ $row['last_poster_name'] = $row['last_poster_name']; }
								if(isset($row['name'])){ $row['name'] = $row['name']; }
								if(isset($row['description'])){ $row['description'] = $row['description']; }
								if(isset($row['last_title'])){ $row['last_title'] = $row['last_title']; }
								if(isset($row['newest_title'])){ $row['newest_title'] = $row['newest_title']; }
								$subforums[$row['id']] = $row;
							}
						}
						// make it rekursive
