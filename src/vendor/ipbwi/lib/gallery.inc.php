<?php
	...
	class ipbwi_gallery extends ipbwi {
		...
		public function getLatestList($catIDs=false,$settings=array()){
			if($this->installed === true){
				if(is_array($catIDs)){
					...
					$catquery = ' AND (image_album_id="'.implode('" OR image_album_id="',$viewable).'")';
				}elseif(intval($catIDs) != 0){
					$catquery = ' AND image_album_id="'.$catIDs.'"';
				}else{
					$catquery = false;
				}
				...
				while($row = $this->ipbwi->ips_wrapper->DB->fetch($query)){
					$row['caption']			= $this->ipbwi->properXHTML($row['caption']);
					$row['description']		= $this->ipbwi->properXHTML($row['description']);
					$row['copyright']		= $this->ipbwi->properXHTML($row['copyright']);
					$data[] = $row;
				}
				return $data;
			}else{
			...
			if($this->installed === true){
				...
				if(is_array($cats) && count($cats) > 0){
					foreach($cats as $i){
						...
						$sql = 'SELECT '.$select.' FROM '.$this->ipbwi->board['sql_tbl_prefix'].'gallery_albums WHERE album_owner_id = '.intval($i['album_id']).' ORDER BY album_position ASC';
		...
		public function catList(){
			if($this->installed === true){
				if($cache = $this->ipbwi->cache->get('listGalleryCategories', '1')){
					...
					$query = $this->ipbwi->ips_wrapper->DB->query('SELECT * FROM '.$this->ipbwi->board['sql_tbl_prefix'].'gallery_albums WHERE album_owner_id="0"');
					$cat = array();
					while($row = $this->ipbwi->ips_wrapper->DB->fetch($query)){
						$row['last_pic_name'] = $this->ipbwi->properXHTML($row['last_pic_name']);
		...
		public function info($imgID){
			if($this->installed === true){
				// get image info
				$query = $this->ipbwi->ips_wrapper->DB->query('SELECT * FROM '.$this->ipbwi->board['sql_tbl_prefix'].'gallery_images WHERE image_id="'.intval($imgID).'"');
				if($this->ipbwi->ips_wrapper->DB->getTotalRows($query) == 0){
					return false;
				}
				$data = array();
