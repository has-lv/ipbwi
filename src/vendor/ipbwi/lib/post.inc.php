<?php
	...
	class ipbwi_post extends ipbwi {
		 ...
		 * @param	bool	$htmlState Default: false = don't parse HTML, true = parse HTML
		 ...
		 * @since			2.0
		 */
		public function create($topicid, $post, $useEmo = true, $useSig = true, $bypassPerms = false, $guestname = false, $htmlState = false, $time = false){
			...
			// get user groups for preview state of post
			$groups = $this->ipbwi->group->listMemberGroups();
			$gPreview = true;
			
			$gInfo = $this->ipbwi->group->info($groups['primary']);
			if($gInfo['g_mod_preview'] != 1){
				$gPreview = false;
			}
			
			if(count($groups['secondary']) > 0){
				foreach($groups['secondary'] as $groupID){
					$gInfo = $this->ipbwi->group->info($groupID);
					if($gInfo['g_mod_preview'] != 1){
						$gPreview = false;
					}
				}
			...
			if($gPreview === true && $this->ipbwi->member->myInfo['gbw_mod_post_unit_type'] == '0' && $this->ipbwi->member->myInfo['posts'] >= $this->ipbwi->member->myInfo['g_mod_post_unit']){
				$gPreview = false;
			}
			...
			if($row = $this->ipbwi->ips_wrapper->DB->fetch()){
				// Check User can Post to Forumvar_dump($preview);
				if($this->ipbwi->forum->isPostable($row['forum_id']) || $bypassPerms){
					// Post Queue based on forum settings
					if(intval($row['preview_posts']) != 0 || intval($this->ipbwi->member->myInfo['mod_posts']) != 0 || $gPreview){
						$preview = 1;
					// Post Queue based on usergroup settings
					}else{
						$preview = 0;
					}
					...
					// POST KEY!
					$this->ipbwi->ips_wrapper->DB->query('INSERT INTO '.$this->ipbwi->board['sql_tbl_prefix'].'posts (author_id, author_name, use_emo, use_sig, ip_address, post_date, post, queued, topic_id, post_key, post_htmlstate) VALUES ("'.$this->ipbwi->member->myInfo['member_id'].'", "'.$postname.'", "'.($useEmo ? 1 : 0).'", "'.($useSig ? 1 : 0).'", "'.$_SERVER['REMOTE_ADDR'].'", "'.$time.'", "'.$post.'", "'.$preview.'", "'.$row['tid'].'", "'.md5(microtime()).'", "'.$htmlState.'")');
					...
					if($this->ipbwi->member->myInfo['member_id'] != '0' && $row['inc_postcount'] && $preview == 0){
		 ...
		 * @since			2.0
		 */
		public function info($postID, $replacePostVars = true, $ipbwiLink = false, $list = false, $topicInfo = false, $topicPost = false){
				if(isset($list['sql'])){
			...
			while($row = $this->ipbwi->ips_wrapper->DB->fetch($sql)){
				...
				if($topicPost == false && $row['new_topic']){
					continue;
				}
				...
				// make proper XHTML
				$post[$row['pid']]										= $row;
				$post[$row['pid']]['post']								= $this->ipbwi->properXHTML($this->ipbwi->ips_wrapper->parser->display($post[$row['pid']]['post']));
				...
				if($replacePostVars === true){
					...
					if(is_array($attachInfo) && count($attachInfo) > 0){
						foreach($attachInfo as $attachList){
							if(isset($attachList['attach_id']) && strpos($post[$row['pid']]['post'],'[attachment='.$attachList['attach_id'].':') != false){
						...
						if(isset($attachInfo['defaultHTML'])){
							$post[$row['pid']]['post'] = preg_replace('/\[attachment=([!0-9]*):([!a-zA-Z0-9 _\-.]*)\]/smeU','$attachInfo["defaultHTML"]',$post[$row['pid']]['post']);
						}else{
							$post[$row['pid']]['post'] = preg_replace('/\[attachment=([!0-9]*):([!a-zA-Z0-9 _\-.]*)\]/smeU','$attachInfo["\1"]["defaultHTML"]',$post[$row['pid']]['post']);
						}
					}
				}
		 ...
		 * + int start						= Default: 0
		 * + int limit						= Default: 15, set to "off" to disable start/limit and return all entries found in database
		 * + int memberid					= optional, load posts from this member only, default: false
		 ...
		 * + string detailedMemberData		= Default: true. Set to false to get a performance boost.
		 ...
		 * @param	bool					$showQueued Default: false = do not list queued or hidden content, true = list queued or hidden content. If user is superMod or admin, queued content always will be viewed.
		 ...
		 * @since			2.0
		 */
		public function getList($topicID, $settings = array(), $bypassPerms = false, $countView = false, $showQueued = false){
			...
			if($settings['detailedMemberData'] !== false){
				$settings['detailedMemberData'] = true;
			}
			if($showQueued == false){
				if($this->ipbwi->member->isSuperMod() && $this->ipbwi->member->isAdmin()){
					$queue = '1=1 ';
				}else{
					$queue = 'p.queued="0" ';
				}
			...
			if($cando){
				...
				if($settings['detailedMemberData'] === true){
					$detailedMemberData			= ' LEFT JOIN '.$this->ipbwi->board['sql_tbl_prefix'].'members m ON (p.author_id=m.member_id)
													LEFT JOIN '.$this->ipbwi->board['sql_tbl_prefix'].'groups g ON (m.member_group_id=g.g_id)';
					$detailedMemberDataSelect	= ', m.*, g.g_dohtml AS usedohtml';
				}else{
					$detailedMemberData			= '';
					$detailedMemberDataSelect	= '';
				}
				
				$query = 'SELECT p.*, t.forum_id, t.title AS topic_name'.$detailedMemberDataSelect.' FROM '.$this->ipbwi->board['sql_tbl_prefix'].'posts p'.$detailedMemberData.' LEFT JOIN '.$this->ipbwi->board['sql_tbl_prefix'].'topics t ON(p.topic_id=t.tid) WHERE p.pid != topic_firstpost AND '.$specificMember.$sqlwhere.$queue.'ORDER BY '.$order.$sql_limit;
